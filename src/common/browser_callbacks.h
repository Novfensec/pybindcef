...
{
  // args: (buffer: memoryview, width: int, height: int,
  //        dirty_rects: list[tuple[int,int,int,int]])  — each rect is (x, y, w, h)
  std::function<void(py::object, int, int, py::list)> on_cpu_paint;

  std::function<void(uint64_t, int, int)> on_gpu_paint;

  /*
  Called before every network request is sent.
  args: (url: str, resource_type: int)
  resource_type mirrors cef_resource_type_t:
    0=main_frame  1=sub_frame  2=stylesheet  3=script  4=image
    5=font  6=sub_resource  7=object  8=media  13=xhr  ...
  Return True to BLOCK (cancel) the request, False to allow it.
  */
  std::function<bool(std::string, int)> on_before_resource_load;
  ...
  */
  ...
  using PaintCb = std::function<void(py::object, int, int, py::list)>;
